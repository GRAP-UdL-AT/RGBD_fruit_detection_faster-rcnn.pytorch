  ...
  for file in files:
    ...
    for IoU in np.array(args.ovthresh).astype(np.float):
        ...
        for confid in np.array(args.minconfid).astype(np.float):
            ...
            if epoch == 0:
              pickle.dump(meanAP_epoch[:,IoUidx,confididx], open('output/meanAP_s'+str(args.checksession)+'_epoch_'+str(epoch+1)+'.pkl','wb'),pickle.HIGHEST_PROTOCOL)

            mat = np.matrix(meanAP_epoch[:,IoUidx,confididx])
            with open('output/meanAP_s'+str(args.checksession)+'_IoU_'+str(IoU)[2:]+'_confid_' +str(confid)[2:]+ '.txt', 'wb') as f:
                for line in mat:
                    np.savetxt(f, line, fmt='%.4f')
            mat = np.matrix(recall[:,IoUidx,confididx])
            with open('output/recall_s'+str(args.checksession)+'_IoU_'+str(IoU)[2:]+'_confid_' +str(confid)[2:]+ '.txt', 'wb') as f:
                for line in mat:
                    np.savetxt(f, line, fmt='%.4f')
            mat = np.matrix(precision[:,IoUidx,confididx])
            with open('output/precision_s'+str(args.checksession)+'_IoU_'+str(IoU)[2:]+'_confid_' +str(confid)[2:]+ '.txt', 'wb') as f:
                for line in mat:
                    np.savetxt(f, line, fmt='%.4f')
            mat = np.matrix(meanTime[:,IoUidx,confididx])
            with open('output/meanTime_s'+str(args.checksession)+'_IoU_'+str(IoU)[2:]+'_confid_' +str(confid)[2:]+ '.txt', 'wb') as f:
                for line in mat:
                    np.savetxt(f, line, fmt='%.4f')
